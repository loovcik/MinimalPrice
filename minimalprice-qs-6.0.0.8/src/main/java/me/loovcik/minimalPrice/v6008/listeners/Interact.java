	...
	private void onInteract(ShopSignUpdateEvent event)
	{
		...
		if (acceptablePrice > oldPrice) {
			Bukkit.getScheduler().runTask(api.plugin, () -> {
				...
				if (api.isDebug.get())
					ChatHelper.console("QSLegacyInteract={material="+event.getShop().getItem().getType()+", price=$"+oldPrice+", acceptablePrice=$"+acceptablePrice+", result=<red>changed</red>}");
			});
		}
		else if (api.isDebug.get())
			ChatHelper.console("QSLegacyInteract={material="+event.getShop().getItem().getType()+", price=$"+event.getShop().getPrice()+", acceptablePrice=$"+acceptablePrice+", result=<green>OK</green>}");
	}
